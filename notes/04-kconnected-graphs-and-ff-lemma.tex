\section{Přednáška 4 -- k-souvislost, Fordova-Fulkersonova věta}
...
\lemma{Pro každý graf $G$ a každé přirozené $t$ palatím že \(k_e \geq k\), právě když mezi každými dvěma různými vrcholy $u$ a $v$ existuje alespoň $t$ hranově (úplně) disjunktních cest.}\label{lemma:ford-fulkersonova-veta}

\subsection{Mengerova věta}

Obdobné tvrzení jako \textit{Ford-Fulkersonova věta}, platí ale pro vrcholovou souvislost.

\lemma{Pro každý graf $G$ a každé přirozené $t$ platí, že \(k_v(G) \geq t\), právě když mezi každými dvěma různými vrcholy $u$ a $v$ existuje alespoň $t$ cest, které jsou až na vrcholy $u$ a $v$ disjunktní.}\label{lemma:mengerova-veta}
